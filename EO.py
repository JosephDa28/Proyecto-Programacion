...
def Prueba():
    ...
    while True:
        origen = valorOrigen.get()
    
        if origen in Lista_Rutas_Primer_Elemento:
            break

    while True:
        destino = valorDestino.get()
    ...
    EntradaConsejo.set("Si desea tomar una ruta directa desde su origen y destino le recomendamos los siguientes buses: {0}".format(Lista_De_Error1))
    ...
    while True:
        ...
        if bus not in lista and bus in Lista_De_Error:
            ...
            EntradaError.set("Este bus no tiene como parada el origen puesto, recuerde que puede tomar alguno de los siguientes buses {0}".format(lista))
    ...
    EntradaDistancia.set("La distancia a recorrer es {0} y el tiempo aproximado es {1} minutos".format(k, int(tiempoTotal)))
...
ventana.title("Tu navegador de Transmilenio")

imagen= PhotoImage(file="bogota1.gif")
fondo = Label(ventana,image=imagen).place(x=0,y=0)



etiqueta = Label(text="Origen:  ",font=("Verdana",20),bg="white").place(x=0,y=0)

etiqueta2 = Label(text="Destino:",font=("Verdana",20),bg="white").place(x=0,y=35)

etiqueta3 = Label(text="Bus:      ",font=("Verdana",20),bg="white").place(x=0,y=70)


EntradaConsejo = StringVar()
#EntradaConsejo.set(str(consejo))

EntradaError = StringVar()
#EntradaError.set(str(error))

EntradaDistancia = StringVar()
#EntradaDistancia.set(dist)


CampoConsejo = Entry(ventana,font=("Verdana",18),textvariable=EntradaConsejo).place(x=0,y=200)
CampoError = Entry(ventana,font=("Verdana",18),textvariable=EntradaError).place(x=0,y=240)
CampoDistancia = Entry(ventana,font=("Verdana",18),textvariable=EntradaDistancia).place(x=0,y=280)


valorBus = StringVar()
valorOrigen = StringVar()
valorDestino = StringVar()


boton = Button(ventana,command=Prueba,text="Buscar",font=("Verdana",17),background="green").place(x=0,y=130)
boton2 = Button(ventana,command=Cancelar,text="Cancelar",font=("Verdana",17),background="red2").place(x=120,y=130)


spnOrigen = Spinbox(ventana,font=("Verdana",20),values=("Calle26","Profamilia","Avenida39","Calle45","Marly","Calle57","Calle63","Flores","Calle72","Calle76","Heroes","Calle85","Virrey","Calle100","Calle106","PepeSierra","Calle127","Prado","Alcala","Mazuren","CardioInfantil","Toberin","PortalNorte","Calle187","Terminal"),textvariable=valorOrigen).place(x=120,y=1)

spnDestino = Spinbox(ventana,font=("Verdana",20),values=("Calle26","Profamilia","Avenida39","Calle45","Marly","Calle57","Calle63","Flores","Calle72","Calle76","Heroes","Calle85","Virrey","Calle100","Calle106","PepeSierra","Calle127","Prado","Alcala","Mazuren","CardioInfantil","Toberin","PortalNorte","Calle187","Terminal"),textvariable=valorDestino).place(x=120,y=36)

spnBus = Spinbox(ventana,font=("Verdana",20),values=("Bus8","BusB74","BusB14","BusB75","BusB18","BusB12","BusB13","BusD20","BusD24","Bus6"),textvariable=valorBus).place(x=120,y=72)
